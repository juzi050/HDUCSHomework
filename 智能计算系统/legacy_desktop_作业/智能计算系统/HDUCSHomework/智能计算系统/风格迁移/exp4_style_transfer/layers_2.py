...
class ConvolutionalLayer(object):
    ...
    def init_param(self, std=0.01):
        self.weight = np.random.normal(loc=0.0, scale=std, size=(self.channel_in, self.kernel_size, self.kernel_size, self.channel_out)).astype(np.float32)
    ...
    def forward_raw(self, input):
        ...
        self.input_pad = np.zeros([self.input.shape[0], self.input.shape[1], height, width], dtype=self.input.dtype)
        ...
        self.output = np.zeros([self.input.shape[0], self.channel_out, height_out, width_out], dtype=self.input.dtype)
        for idxn in range(self.input.shape[0]):
            for idxc in range(self.channel_out):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        window = self.input_pad[idxn, :, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size]
                        self.output[idxn, idxc, idxh, idxw] = np.sum(window * self.weight[:, :, :, idxc]) + self.bias[idxc]
    ...
    def forward_speedup(self, input):
        ...
        input_pad = np.ascontiguousarray(self.input_pad)
        windows = np.lib.stride_tricks.sliding_window_view(input_pad, (self.kernel_size, self.kernel_size), axis=(2, 3))
        windows = windows[:, :, ::self.stride, ::self.stride, :, :]
        self.img2col = windows.transpose([0, 2, 3, 1, 4, 5]).reshape([-1, self.channel_in*self.kernel_size*self.kernel_size])
    ...
    def backward_speedup(self, top_diff):
        start_time = time.time()
        bottom_diff = np.zeros(self.input_pad.shape, dtype=self.input.dtype)
        ...
        self.d_bias = np.dot(np.ones([1, self.input.shape[0]*self.height_out*self.width_out], dtype=top_diff.dtype), top_diff).reshape(-1)
        for idxn in range(self.input.shape[0]):
    ...
    def backward_raw(self, top_diff):
        start_time = time.time()
        self.d_weight = np.zeros(self.weight.shape, dtype=self.weight.dtype)
        self.d_bias = np.zeros(self.bias.shape, dtype=self.bias.dtype)
        bottom_diff = np.zeros(self.input_pad.shape, dtype=self.input.dtype)
            ...
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        window = self.input_pad[idxn, :, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size]
                        self.d_weight[:, :, :, idxc] += window * top_diff[idxn, idxc, idxh, idxw]
...
class MaxPoolingLayer(object):
    ...
    def forward_raw(self, input):
        start_time = time.time()
        self.input = input  # [N, C, H, W]
        self.max_index = np.zeros(self.input.shape, dtype=np.int32)
        height_out = (self.input.shape[2] - self.kernel_size) // self.stride + 1
        width_out = (self.input.shape[3] - self.kernel_size) // self.stride + 1
        self.output = np.zeros([self.input.shape[0], self.input.shape[1], height_out, width_out], dtype=self.input.dtype)
        for idxn in range(self.input.shape[0]):
            for idxc in range(self.input.shape[1]):
                for idxh in range(height_out):
                    for idxw in range(width_out):
                        ...
                        curren_max_index = np.argmax(window)
    ...
    def forward_speedup(self, input):
        ...
        input_view = np.ascontiguousarray(self.input)
        windows = np.lib.stride_tricks.sliding_window_view(input_view, (self.kernel_size, self.kernel_size), axis=(2, 3))
        windows = windows[:, :, ::self.stride, ::self.stride, :, :]
        img2col = windows.reshape([self.input.shape[0], self.input.shape[1], self.height_out*self.width_out, self.kernel_size*self.kernel_size])
        ...
        self.max_index = np.zeros([self.argmax.shape[0], img2col.shape[-1]], dtype=self.input.dtype)
    ...
    def backward_speedup(self, top_diff):
        bottom_diff = np.zeros(self.input.shape, dtype=self.input.dtype)
        ...
        for idxn in range(top_diff.shape[0]):
            for idxc in range(top_diff.shape[1]):
                for idxh in range(top_diff.shape[2]):
                    for idxw in range(top_diff.shape[3]):
                        window = self.input[idxn, idxc, idxh*self.stride:idxh*self.stride+self.kernel_size, idxw*self.stride:idxw*self.stride+self.kernel_size]
                        max_index = np.unravel_index(np.argmax(window), [self.kernel_size, self.kernel_size])
