...
class FullyConnectedLayer(object):
    ...
    def update_param(self, lr, momentum=0.0, weight_decay=0.0):  # 参数更新
        # TODO：对全连接层参数利用参数进行更新
        if self.weight_velocity is None:
            self.weight_velocity = np.zeros_like(self.weight)
            self.bias_velocity = np.zeros_like(self.bias)
        self.weight_velocity = momentum * self.weight_velocity - lr * (self.d_weight + weight_decay * self.weight)
        self.bias_velocity = momentum * self.bias_velocity - lr * self.d_bias
        self.weight = self.weight + self.weight_velocity
        self.bias = self.bias + self.bias_velocity
...
class SoftmaxLossLayer(object):
    ...
    def get_loss(self,label):  # 计算损失
        ...
        loss=-np.sum(np.log(np.clip(self.prob, 1e-12, 1.0))*self.label_onehot)/self.batch_size
        return loss
