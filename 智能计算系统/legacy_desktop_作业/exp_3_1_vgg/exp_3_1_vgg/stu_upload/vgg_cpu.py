...
class VGG19(object):
    ...
    def load_model(self):
        ...
        # 兼容两种mat结构:
        # 1) 旧版: params['normalization']
        # 2) 新版: params['meta'][0,0]['normalization'][0,0]['averageImage']
        if 'normalization' in params:
            self.image_mean = params['normalization'][0][0][0]
        else:
            normalization = params['meta'][0, 0]['normalization'][0, 0]
            self.image_mean = normalization['averageImage']
        self.image_mean = np.mean(self.image_mean, axis=(0, 1))
        print('Get image mean: ' + str(self.image_mean))

        def get_weight_bias(layer_idx):
            layer_raw = params['layers'][0][layer_idx]
            # 新版struct格式
            try:
                layer_struct = layer_raw[0, 0]
                if layer_struct.dtype.names is not None and 'weights' in layer_struct.dtype.names:
                    weight, bias = layer_struct['weights'][0]
                    return weight, bias
            except Exception:
                pass
            # 旧版cell嵌套格式
            weight, bias = layer_raw[0][0][0][0][0][0]
            return weight, bias

        for idx in range(43):
            if 'conv' in self.param_layer_name[idx]:
                weight, bias = get_weight_bias(idx)
            ...
            if idx >= 37 and 'fc' in self.param_layer_name[idx]:
                weight, bias = get_weight_bias(idx - 1)
    ...
    def load_image(self, image_dir):
        ...
        # mimic scipy.misc.imresize to fixed size 224x224
        h, w = self.input_image.shape[:2]
        sh = np.linspace(0, h - 1, 224).round().astype(np.int32)
        sw = np.linspace(0, w - 1, 224).round().astype(np.int32)
        self.input_image = self.input_image[sh][:, sw]
